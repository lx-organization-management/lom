...
public interface LomEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/lom */
    ...
    /** The key of the configuration. e.g. returnpath@lom.luxa.jp */
     ...
     * The value is, e.g. /tmp/lastaflute/lom <br>
     ...
     * The value is, e.g. returnpath@lom.luxa.jp <br>
     ...
     * The value is, e.g. lom-support@annie.example.com <br>
     * comment: Mail Address for Lom Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements LomEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(LomEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(LomEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(LomEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(LomEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(LomEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(LomEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(LomEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(LomEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(LomEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(LomEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(LomEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(LomEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(LomEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(LomEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(LomEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(LomEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(LomEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(LomEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(LomEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(LomEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(LomEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(LomEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(LomEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

...
import jp.luxa.lom.mylasta.direction.LomEnv;
import org.lastaflute.core.direction.exception.ConfigPropertyNotFoundException;

/**
 * @author FreeGen
 */
public interface LomConfig extends LomEnv {

    /** The key of the configuration. e.g. lom */
    ...
    /**
     * Get the value for the key 'cookie.remember.me.lom.key'. <br>
     * The value is, e.g. HRB <br>
     * comment: The cookie key of remember-me for Lom #change_it_first
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
    String getCookieRememberMeLomKey();

    /**
     * The simple implementation for configuration.
     * @author FreeGen
     */
    public static class SimpleImpl extends LomEnv.SimpleImpl implements LomConfig {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getDomainName() {
            return get(LomConfig.DOMAIN_NAME);
        }

        public String getDomainTitle() {
            return get(LomConfig.DOMAIN_TITLE);
        }

        public String getAppStandardDatePattern() {
            return get(LomConfig.APP_STANDARD_DATE_PATTERN);
        }

        public String getAppStandardDatetimePattern() {
            return get(LomConfig.APP_STANDARD_DATETIME_PATTERN);
        }

        public String getAppStandardTimePattern() {
            return get(LomConfig.APP_STANDARD_TIME_PATTERN);
        }

        public String getCookieDefaultPath() {
            return get(LomConfig.COOKIE_DEFAULT_PATH);
        }

        public String getCookieDefaultExpire() {
            return get(LomConfig.COOKIE_DEFAULT_EXPIRE);
        }

        public Integer getCookieDefaultExpireAsInteger() {
            return getAsInteger(LomConfig.COOKIE_DEFAULT_EXPIRE);
        }

        public String getCookieEternalExpire() {
            return get(LomConfig.COOKIE_ETERNAL_EXPIRE);
        }

        public Integer getCookieEternalExpireAsInteger() {
            return getAsInteger(LomConfig.COOKIE_ETERNAL_EXPIRE);
        }
